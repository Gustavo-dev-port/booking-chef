...
export default function RecipeCostHistoryScreen() {
  ...
  return (
    <View className="flex-1 bg-surface-page dark:bg-surface-page-dark pt-16">
      <View className="px-4">
        <BackButton />
        <AppText className="mb-4 text-2xl font-archivo-bold text-ink dark:text-ink-dark">Histórico de custo</AppText>
      </View>
        ...
        renderItem={({ item }) => (
          <View className="mb-2 rounded-xl border border-surface-border dark:border-surface-border-dark bg-surface-card dark:bg-surface-card-dark p-3">
            <View className="flex-row items-center justify-between">
              ...
              <AppText className="text-xs text-ink-secondary dark:text-ink-secondary-dark">{formatDateTime(item.calculated_at)}</AppText>
            </View>
            <AppText className="mt-0.5 text-sm text-ink-secondary dark:text-ink-secondary-dark">
